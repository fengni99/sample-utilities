...
import com.discovery.darchrow.http.entity.HttpHeaders;
import com.discovery.darchrow.http.entity.RequestAttributes;
import com.discovery.darchrow.http.entity.RequestLogSwitch;
...
import com.discovery.darchrow.net.URIComponents;
...
public final class RequestUtil{
    ...
    public static final String getClientIp(HttpServletRequest request){
        ...
        // 程哥说放置在X-Real-IP中（2015-12-11 16:12）
        String ipAddress = request.getHeader(HttpHeaders.X_REAL_IP);
        map.put("0.header_xRealIp", ipAddress);
    ...
    /**
     * 判断当前请求是否是https, 这个判断是基于header中的变量scheme的，需要运维在nginx中设置
     * @param request
     *            当前请求
     * @return
     */
    public static boolean isHttpsRequest(HttpServletRequest request) {
    	...
    	return scheme != null && scheme.endsWith("https");
    }
    
    /**
     * 取得当前请求的Root Path，scheme+port+getContextPath，scheme读取nginx中header的配置
     * @param request
     *            当前请求
     * @return
     */
    public static String getServerRootWithNginxScheme(HttpServletRequest request) {
    	String serverRootWithContextPath = getServerRootWithContextPath(request);
		if(isHttpsRequest(request) && !serverRootWithContextPath.startsWith("https") ){
			serverRootWithContextPath = "https" + serverRootWithContextPath.substring(serverRootWithContextPath.indexOf(":"));
		}
    	return serverRootWithContextPath;
    }
}
